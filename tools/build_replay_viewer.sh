...
test -s "${dist}/eleusis_replay.wasm"
test -s "${dist}/eleusis_replay.js"

# ecos, 2026-08-23: `coworld build` pre-creates the bundle directory's
# parent, and CI does not. Create it BEFORE anything touches the path, or the
# hook exits 1 on a fresh checkout.
mkdir -p "$(dirname "${output_dir}")"
rm -rf "${output_dir}"
mkdir -p "${output_dir}/assets"
cp "${dist}/eleusis_replay.js" "${dist}/eleusis_replay.wasm" "${output_dir}/"
